@php
    ...
    $pageItem = __('trans.open purchase order')
            ...
            <div>
                <div class="card card-success">
                    <div class="error_messages">
                        ...
                        <div class="card-body">
                            {!! Form::open(['route' => ['admin.openPurchaseOrders.update', $openPurchaseOrder -> id], 'method' => 'put', 'id' => 'openPurchaseOrder']) !!}
                            {!! Form::hidden('supplier_id', $openPurchaseOrder -> supplier_id) !!}
                            {!! Form::hidden('invoice_date', $openPurchaseOrder -> invoice_date) !!}
                            {!! Form::hidden('branch_id', $openPurchaseOrder -> branch_id) !!}
                            <div class="col-xs-12 col-custom-style">
                                <div class="row">
                                    <div class="col-xs-12 col-md-3">
                                        {!! Form::label('invoice_number', __('trans.invoice number'), ['class' => 'control-label']) !!}
                                        {!! Form::text('invoice_number', $openPurchaseOrder -> invoice_number , ['class' => 'form-control invoice_number']) !!}
                                    </div>
...
{{--                                    </div>--}}
                            ...
                            <div style="margin: 0; padding: 0" class="col-xs-12 col-md-9 col-custom-style">
                                <div class="purchase_order_products" style="border: #0a0a0a 1px solid; height: 400px; overflow-y: scroll; padding: 5px; margin-top: 10px">
                                    <table class="table" id="purchase_order_products">
                                        ...
                                        <tbody>
                                        @foreach ($openPurchaseOrder -> openPurchaseOrderProducts -> sortByDesc('id') as $key => $product)
                            ...
                            <div class="col-xs-12 col-custom-style">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-2">
                                        <input readonly type="text" name="total" class="form-control total" id="total" value="{{ $openPurchaseOrder -> total }}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="discount" class="col-sm-4 col-form-label">مجموع الخصومات</label>
                                    <div class="col-sm-2">
                                        <input readonly type="text" name="discount" class="form-control discount" id="discount" value="{{ $openPurchaseOrder -> discount }}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="taxable_amount" class="col-sm-4 col-form-label">الاجمالى الخاضع للضريبة (غير شامل ضريبة القيمة المضافة)</label>
                                    <div class="col-sm-2">
                                        <input readonly type="text" name="taxable_amount" class="form-control taxable_amount" id="taxable_amount" value="{{ $openPurchaseOrder -> taxable_amount }}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="total_vat" class="col-sm-4 col-form-label">مجموع ضريبة القيمة المضافة</label>
                                    <div class="col-sm-2">
                                        <input readonly type="text" name="total_vat" class="form-control total_vat" id="total_vat" value="{{ $openPurchaseOrder -> total_vat }}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="total_amount_due" class="col-sm-4 col-form-label">اجمالى المبلغ المستحق</label>
                                    <div class="col-sm-2">
                                        <input readonly type="text" name="total_amount_due" class="form-control total_amount_due" id="total_amount_due" value="{{ $openPurchaseOrder -> total_amount_due }}">
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="form-group">
                                        <input class="payment_method" type="checkbox" name="payment_method" id="cash" value="كاش" {{ $openPurchaseOrder -> payment_method ? 'checked': '' }}>
                                        <label for="cash">كاش</label>
                                    </div>

                                    <div class="form-group">
                                        <input class="payment_method" type="checkbox" name="payment_method_bank" id="bank_transfer" value="تحويل بنكى" {{ $openPurchaseOrder -> payment_method_bank == 'تحويل بنكى' ? 'checked': '' }}>
                                        <label for="bank_transfer">تحويل بنكى</label>

                                        <input class="payment_method" type="checkbox" name="payment_method_bank" id="network" value="شبكة" {{ $openPurchaseOrder -> payment_method_bank == 'شبكة' ? 'checked': '' }}>
                                        <label for="network">شبكة</label>

                                        <input class="payment_method" type="checkbox" name="payment_method_bank" id="stc_pay" value="STC-Pay" {{ $openPurchaseOrder -> payment_method_bank == 'STC-Pay' ? 'checked': '' }}>
                                ...
                                <div id="amounts_group_field">
                                    @if ($openPurchaseOrder -> amount_paid)
                                        <div id="safe_amount_field" class="form-group row">
                                            ...
                                            <div class="col-sm-2">
                                                <input type="text" name="amount_paid" class="form-control amount_paid" id="amount_paid" value="{{ $openPurchaseOrder -> amount_paid }}" autocomplete="off">
                                            </div>
                                        </div>
                                    @endif
                                    @if ($openPurchaseOrder -> amount_paid_bank)
                                        <div id="bank_field" class="form-group row">
                                            ...
                                            <div class="col-sm-2">
                                                <input type="text" name="amount_paid_bank" class="form-control amount_paid_bank" id="amount_paid_bank" value="{{ $openPurchaseOrder -> amount_paid_bank }}" autocomplete="off">
                                            </div>
                                        </div>
                                    @endif
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-2">
                                        <input readonly type="text" name="amount_due" class="form-control amount_due" id="amount_due" value="{{ $openPurchaseOrder -> amount_due }}">
                                    </div>
                            ...
                            <div class="col-xs-12 col-custom-style">
                                <div class="form-group">
                                    ...
                                    <textarea name="notes" id="note" rows="5" class="form-control" placeholder="ملاحظات">{{ $openPurchaseOrder -> notes }}</textarea>
                                </div>
                            </div> <!-- end col 12 -->
                            <div class="col-xs-12 col-custom-style">
