@php
    ...
    $pageItem = __('trans.purchase order return')
@endphp
            ...
            <div>
                <div class="card card-success">
                    <div class="error_messages">
                        ...
                        <div class="card-body">
                            {!! Form::open(['route' => 'admin.purchaseOrderReturns.store', 'method' => 'post', 'id' => 'purchase_order_returns']) !!}
{{--                            {!! Form::hidden('invoice_date', $purchaseOrder -> invoice_date) !!}--}}
                            {!! Form::hidden('branch_id', $purchaseOrder -> branch_id) !!}
{{--                            {!! Form::hidden('client_id', $purchaseOrder -> client_id, ['id' => 'client_id']) !!}--}}
{{--                            {!! Form::hidden('check_id', $purchaseOrder -> check_id) !!}--}}
                            {!! Form::hidden('purchase_order_id', $purchaseOrder -> id) !!}
                            <div style="margin: 0; padding: 0" class="col-xs-12 col-custom-style">
                                <div class="purchase_order_return_products" style="width: 100%; margin: 0 auto">
                                    <table style="width: 100%" class="table">
                                        ...
                                        <tbody>
                                        @foreach ($purchaseOrderProducts -> sortByDesc('id') as $key => $product)
                                            <tr>
                                                <td>
...
@push('scripts')
    ...
    <script>
        ...
        // JQuery Validation
        $("#purchase_order_returns").validate({
            // rules: {
        ...
        $(document).on('change', 'input[type="checkbox"]', function () {
            ...
            if(that.is(':checked'))
            {
                let content = '<td class="return_amount_in_row">' +
                    ...
                    '<option value="supplier_balance">آجل</option>' +
                    '</select>' +
                    '</td>';
