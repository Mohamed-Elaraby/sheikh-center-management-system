@if (Auth::user()->hasPermission('update-carModel'))
    <a href="{{ route('admin.carModel.edit', $query->id) }}" class="btn btn-sm btn-primary"> <i class="fa fa-edit"></i> {{ __('trans.edit') }}</a>
@endif
@if (Auth::user()->hasPermission('delete-carModel'))
    {!! Form::open(['route' => ['admin.carModel.destroy', $query->id], 'method' => 'delete', 'style' => 'display:inline']) !!}
