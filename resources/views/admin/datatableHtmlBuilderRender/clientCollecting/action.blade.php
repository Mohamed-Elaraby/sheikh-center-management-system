@if (Auth::user()->hasPermission('update-clientCollecting'))
    <a href="{{ route('admin.clientCollecting.edit', $query->id) }}" class="btn btn-sm btn-primary"> <i class="fa fa-edit"></i> {{ __('trans.edit') }}</a>
@endif
@if (Auth::user()->hasPermission('delete-clientCollecting'))
    {!! Form::open(['route' => ['admin.clientCollecting.destroy', $query->id], 'method' => 'delete', 'style' => 'display:inline']) !!}
