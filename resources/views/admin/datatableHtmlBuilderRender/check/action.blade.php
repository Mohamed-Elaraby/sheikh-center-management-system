...
<select name="" id="selectAction" class="form-control">
    ...
    <option value="{{ route('admin.check.createDeviceReport', [$query->id, $query->check_number]) }}">رفع تقرير حالة</option>
    <option value="{{ route('admin.check.images', [$query->id, 2]) }}">عرض تقرير حالة</option>
    <option value="{{ route('admin.check.clientSignature', [$query->id, $query->check_number,'exit=true']) }}">تسجيل خروج السيارة من المركز</option>
    @if (Auth::user()->hasPermission('create-managementNotes'))
