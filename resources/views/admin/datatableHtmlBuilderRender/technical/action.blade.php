@if (Auth::user()->hasPermission('update-technicals'))
    <a href="{{ route('admin.technicals.edit', $query->id) }}" class="btn btn-sm btn-primary"> <i class="fa fa-edit"></i> {{ __('trans.edit') }}</a>
@endif
@if (Auth::user()->hasPermission('delete-technicals'))
    {!! Form::open(['route' => ['admin.technicals.destroy', $query->id], 'method' => 'delete', 'style' => 'display:inline']) !!}
