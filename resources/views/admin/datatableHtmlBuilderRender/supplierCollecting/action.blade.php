@if (Auth::user()->hasPermission('update-supplierCollecting'))
    <a href="{{ route('admin.supplierCollecting.edit', $query->id) }}" class="btn btn-sm btn-primary"> <i class="fa fa-edit"></i> {{ __('trans.edit') }}</a>
@endif
@if (Auth::user()->hasPermission('delete-supplierCollecting'))
    {!! Form::open(['route' => ['admin.supplierCollecting.destroy', $query->id], 'method' => 'delete', 'style' => 'display:inline']) !!}
