@php
    $pageItem = __('trans.supplier transactions')
@endphp
@extends('admin.layouts.app')

@section('title', $supplier -> name . ' | الرصيد النهائى '. number_format($supplier -> balance, 2) )
        ...
        <div class="col-xs-12">
            <div class="card">
                ...
                <div class="card-header" style="margin: 20px 0">
                    <div class="row">
                        <div class="col-xs-12">
                            <h3 class="text-center"><i class="fa fa-exchange"></i> {{ $pageItem . ' [ '. $supplier -> name.' ] '}}</h3>
                            <h4 class="text-center">
                                <i class="fa fa-money"></i> {{ __('trans.balance') .' [ '.number_format($supplier -> balance, 2).' ] '}}

                                @if (Auth::user()->hasPermission('create-supplierPayments'))
                                    <a href="{{ route('admin.supplierPayments.create', ['supplier_id' => $supplier->id]) }}">{{ __('trans.pay')}}</a>
                                @endif
                                |
                                @if (Auth::user()->hasPermission('create-supplierCollecting'))
                                    <a href="{{ route('admin.supplierCollecting.create', ['supplier_id' => $supplier->id]) }}">{{ __('trans.collect')}}</a>
                                @endif
...
@push('scripts')
    ...
    <script>
        ...
        $('#filter_date_button').on('click', function () {
            ...
            let table = $('#suppliertransactiondatatable-table');
