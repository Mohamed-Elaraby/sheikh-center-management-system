            ...
            <div class="card">
                ...
                <div class="card-header" style="margin: 20px 0">
                    <div class="row">
                        <div class="col-xs-6">
                            <div class="row">
                                <div class="col-xs-6">
                                    <h3 style="margin: 0; padding: 0; display: inline"> <i class="fa fa-pencil-square-o"></i> {{ __('trans.employee signature') . ' ' . $discount -> employee -> name }}</h3>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="text-center">
                        <h2 style="margin-bottom: 5vw">خصم</h2>
...
@push('scripts')
    ...
    <script>
        ...
        {
            var redirect_url = '{{ route('employee.discount.receipt.view', $discount -> id) }}';
            let discount_id = '{{ $discount -> id }}';
            let employee_name = '{{ $discount -> employee -> name }}';
            let canvas_image_data = sig.jSignature('getData');
            let image_data = canvas_image_data.replace(/^data:image\/(png|jpg);base64,/, "");
            let url = '{{ route('employee.discount.employee_signature') }}';
            console.log('Good')
            $.ajax({
                headers: {
                ...
                type: "POST",
                url: url,
                data: {image_data:image_data, discount_id: discount_id, employee_name: employee_name},
                dataType: 'json',
