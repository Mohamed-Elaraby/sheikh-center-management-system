...
@section('title', __('trans.all discounts list'))
        ...
        <div class="col-xs-12">
            <div class="card">
                ...
                <div class="card-header" style="margin: 20px 0">
                    <div class="row">
                        <div class="col-xs-6">
                            <h3 style="margin: 0; padding: 0; display: inline">{{ __('trans.all discounts list') . ' ' . $employeeName}}</h3>
                        </div>
                        <div class="col-xs-6">
                            @if(request('employee_id'))
                                @if (Auth::user()->hasPermission('create-discounts'))
                                    <a href="{{ route('employee.discounts.create', ['employee_id' => request()->get('employee_id')]) }}" class="btn btn-success btn-sm pull-right"><i class="fa fa-plus"></i> {{ __('trans.create') }}</a>
                                @endif
                            @endif
                        </div>
                    </div>
