...
class CategoryDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($query){
                return view('admin.datatableHtmlBuilderRender.category.action', compact('query'));
            })
            ->rawColumns(['action']);
    }
    ...
    public function query(Category $model, Request $request)
    {
        ...
        $category = $model->newQuery();

        if (!empty($start_date) && !empty($end_date)) {
            $category = $model->newQuery()->whereBetween('updated_at', $date_range);
        }
