...
class ClientPaymentDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($query){
                return view('admin.datatableHtmlBuilderRender.clientPayment.action', compact('query'));
            })
     ...
     *
     * @param \App\Models\ClientPayment $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(ClientPayment $model)
    {
        // Search Parameters
        ...
        $clientPayment = $model->newQuery();

        if (!empty($start_date) && !empty($end_date)) {
            $clientPayment = $model->newQuery()->whereBetween('updated_at', $date_range);
        }
    ...
    {
        return $this->builder()
                    ->setTableId('clientpaymentdatatable-table')
...
//            Column::make('action')              -> title( __('trans.action') )
    ...
    {
        return 'ClientPayment_' . date('YmdHis');
    }
}
